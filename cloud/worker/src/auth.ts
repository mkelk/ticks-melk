...
export async function listTokens(env: Env, userId: string): Promise<Response> {
  const result = await env.DB.prepare(
    "SELECT id, name, last_used_at, created_at, revoked_at FROM tokens WHERE user_id = ? ORDER BY created_at DESC"
  )
    .bind(userId)
    .all<Token>();
...
export async function revokeToken(
  ...
  const result = await env.DB.prepare(
    "UPDATE tokens SET revoked_at = unixepoch() WHERE id = ? AND user_id = ? AND revoked_at IS NULL"
  )
    .bind(tokenId, userId)
    .run();
...
export async function validateToken(
  ...
  const result = await env.DB.prepare(
    "SELECT t.id, t.user_id FROM tokens t WHERE t.token_hash = ? AND t.revoked_at IS NULL"
  )
    .bind(tokenHash)
  ...
  // Update last used
  await env.DB.prepare("UPDATE tokens SET last_used_at = unixepoch() WHERE id = ?")
    .bind(result.id)
    .run();
